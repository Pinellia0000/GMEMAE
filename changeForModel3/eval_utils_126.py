...
def calculate_proposal_with_score(opt, array_score_start, array_score_end,
    ...
    if exp == 2:  # 微表情
        ...
        max_len = opt['macro_max']
        apex_score_threshold = opt["macro_apex_score_threshold"]
    else:
        raise ValueError

    apex_indices = np.nonzero(
        array_score_apex > apex_score_threshold)  # (#apex, 2=batch_index + window_index) : score: (B, T)
...
def eval_single_epoch(opt, model, dataloader, epoch, device):
    ...
    for (feature, offset, vid_name) in dataloader:
        ...
        total_inference_time += inference_time  # 累加每个batch的推理时间
    ...
    # 打印总的推理时间
    print(f"Total Inference Time for Epoch {epoch + 1}: {total_inference_time:.6f} seconds")
    inference_time_csv = os.path.join(opt['output_dir_name'], 'inference_time.csv')
    if os.path.exists(inference_time_csv):
        with open(inference_time_csv, 'a', newline='') as file:
            writer_csv = csv.writer(file)
            writer_csv.writerow([epoch + 1, total_inference_time])  # 保存轮次和推理时间
    else:
        # 否则创建后写入
        with open(inference_time_csv, 'w', newline='') as file:
            writer_csv = csv.writer(file)
